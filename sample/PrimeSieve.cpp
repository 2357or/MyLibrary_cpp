...
int main(){
    int n; scanf("%d", &n);
    PrimeSieve P(max(100000, n+1));

    cout << "isPrime: " << (P.isPrime(n) ? "Yes" : "No") << endl;

    cout << "factorize" << endl;
    auto list = P.factorize(n);
    for(int& x : list) cout << ' ' << x;
    cout << endl;

    cout << "divisorCount: " << P.divisorCount(n) << endl;

    cout << "divisors" << endl;
    auto divs = P.divisors(n);
    for(int& x : divs) cout << ' ' << x;
    cout << " (" << divs.size() << ")" << endl;

    //for(int& x : P.primes) printf("%d ", x);
    //puts("");
}
